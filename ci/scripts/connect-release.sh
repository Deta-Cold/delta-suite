...
echo "Uploading to s3://connect.detahard.io/$latest_version/ and s3://connect.detahard.io/$current_version/"
...
aws s3 sync --delete --cache-control 'public, max-age=3600' "$tmp_folder/" "s3://connect.detahard.io/$latest_version/"
aws s3 sync --delete --cache-control 'public, max-age=3600' "$tmp_folder/" "s3://connect.detahard.io/$current_version/"
